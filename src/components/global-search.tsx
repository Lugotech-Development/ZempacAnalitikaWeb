...
export function GlobalSearch() {
  ...
  const [sucursales, setSucursales] = useState<Sucursal[]>(
    () => getCached<Sucursal[]>("sucursales")?.data ?? []
  );
  const [productos, setProductos] = useState<RptProductoMasVendido[]>(
    () => getCached<RptProductoMasVendido[]>("rpt:productos")?.data ?? []
  );
  ...
  const ensureIndex = () => {
    ...
    Promise.allSettled([
      fetchAndCache("sucursales", apiSucursales),
      fetchAndCache("rpt:productos", apiProductos),
    ]).then((results) => {
  ...
  const INDEX: Hit[] = useMemo(() => {
    return [
      ...
      ...productos.map<Hit>((p) => ({
        ...
        href: p.producto
          ? `/dashboard/productos?sku=${encodeURIComponent(p.producto)}`
          : "/dashboard/productos",
        icon: "shopping_bag",
      })),
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-surface-lowest border border-surface-mid focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/15 transition">
        ...
        />
        <kbd className="hidden sm:inline-flex items-center gap-0.5 text-[10px] font-bold text-outline bg-surface-low border border-surface-mid rounded px-1.5 py-0.5">
          ⌘K
        </kbd>
      </div>
                ...
                return (
                  <li key={`${hit.type}-${hit.title}-${i}`}>
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3 py-2 mx-1 rounded-lg transition ${
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {hit.subtitle && (
                          <p className="text-[11px] text-ink-variant truncate">
                            {hit.subtitle}
                          </p>
                        )}
                      </div>
                      <span className="text-[10px] font-bold uppercase tracking-eyebrow text-outline shrink-0">
                        {hit.type}
                      </span>
                    </Link>
                  </li>
