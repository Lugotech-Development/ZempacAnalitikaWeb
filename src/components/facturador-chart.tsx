  ...
  return (
    <div style={{ height: chartHeight }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart layout="vertical" data={data} margin={{ top: 4, right: 90, left: 0, bottom: 4 }} barCategoryGap="30%">
          ...
          <Bar dataKey="total" radius={[0, 4, 4, 0]} maxBarSize={22}>
            {data.map((entry, i) => (
              <Cell key={i} fill={TOKEN.primary} fillOpacity={1 - (i / data.length) * 0.35} />
            ))}
            <LabelList dataKey="total" position="right" formatter={(v: number) => fmtMoney(v)} style={{ fontSize: 11, fontWeight: 700, fill: TOKEN.outline }} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </div>
