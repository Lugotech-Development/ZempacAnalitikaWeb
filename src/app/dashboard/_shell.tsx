    ...
    <div className="min-h-screen bg-surface">
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute inset-y-0 left-0 w-72 bg-surface-lowest shadow-card">
            <div className="flex items-center justify-between px-5 py-4 border-b border-surface-mid">
              ...
              <button
                onClick={() => setMobileOpen(false)}
                className="p-2 rounded-lg hover:bg-surface-low"
            ...
            <button
              onClick={logout}
              className="mx-3 mt-4 flex w-[calc(100%-1.5rem)] items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-tertiary hover:bg-tertiary/5"
            >
      ...
      <div className="lg:pl-72">
        <Topbar
          ...
          onLogout={logout}
        />
        <main className="mx-auto max-w-6xl px-5 sm:px-8 py-6 sm:py-10">
          {children}
        </main>
      </div>
    </div>
  ...
  return (
    <aside className="hidden lg:flex fixed inset-y-0 left-0 w-72 flex-col bg-surface-lowest border-r border-surface-mid">
      ...
      <div className="px-4 pt-4">
        ...
        <p className="mt-1 px-2 text-sm font-bold text-ink truncate">
          {session.empresa || "—"}
        </p>
      </div>
      ...
      <div className="p-3 border-t border-surface-mid">
        <button
          onClick={onLogout}
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-tertiary hover:bg-tertiary/5"
        >
          <Icon name="logout" size={16} />
          Cerrar sesión
        </button>
            ...
            <Link
              ...
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${
                active
                  ...
                  : "text-ink-variant hover:bg-surface-low hover:text-ink"
              }`}
            >
  ...
  return (
    <header className="sticky top-0 z-20 bg-surface-low/80 backdrop-blur border-b border-surface-mid">
      <div className="flex items-center gap-3 sm:gap-4 px-5 sm:px-8 py-3">
        <button
          onClick={onMenu}
          className="lg:hidden p-2 rounded-lg hover:bg-surface-mid"
        ...
        <div className="ml-auto flex items-center gap-1 sm:gap-2">
          <button
            onClick={onMenu}
            className="md:hidden p-2 rounded-lg hover:bg-surface-mid"
            aria-label="Buscar"
          >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-pill hover:bg-surface-mid"
              aria-label="Menú de usuario"
              aria-expanded={menuOpen}
            >
              <span className="h-8 w-8 rounded-full bg-primary-gradient text-white flex items-center justify-center text-sm font-bold">
                {initials}
              </span>
            ...
            {menuOpen && (
              <div
                className="absolute right-0 mt-2 w-56 card-bordered p-2 z-10"
              ...
              >
                ...
                <button
                  onClick={onLogout}
                  className="mt-1 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold text-tertiary hover:bg-tertiary/5"
                >
