    ...
    <header className="sticky top-0 z-30 backdrop-blur bg-surface/80 border-b border-surface-mid">
      <div className="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-semibold text-ink-variant">
          ...
          <a href="#reportes" className="hover:text-ink">Reportes</a>
      ...
      <div className="mx-auto max-w-6xl px-6 pt-20 pb-24 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <h1 className="text-5xl sm:text-6xl font-extrabold tracking-tight text-ink leading-[1.05]">
            Reportes que <span className="text-primary">impulsan</span> tu
            negocio.
          </h1>
          <p className="mt-6 text-lg text-ink-variant leading-relaxed max-w-xl">
            Zempac Analitika centraliza tus ventas, devoluciones, productos y
            cuadres de caja en una sola plataforma. Decisiones más rápidas,
            con datos en tiempo real desde cualquier sucursal.
          </p>
          ...
          <div className="mt-10 flex flex-wrap gap-6 text-sm text-ink-variant">
            {["Sin instalaciones", "Datos en vivo", "Multi-sucursal"].map(
              (t) => (
    ...
    <div className="relative">
      <div className="card p-6 rotate-1">
        <div className="rounded-2xl bg-primary-gradient p-6 text-white">
          <p className="text-[11px] font-bold tracking-eyebrow uppercase opacity-80">
            Ventas Mes Actual
          </p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          {[
            ...
            <div
              key={x.l}
              className="rounded-2xl bg-surface-low p-3"
            >
  ...
  return (
    <section className="border-y border-surface-mid bg-surface-lowest">
      <div className="mx-auto max-w-6xl px-6 py-12">
        <p className="eyebrow text-center text-outline">
          Empresas que confían en Zempac Analitika
        </p>
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {clients.map((c) => (
            <div
              key={c.name}
              className="flex items-center gap-3 rounded-2xl border border-surface-mid bg-surface px-4 py-3 transition hover:border-surface-high"
            >
              <div
                className={`h-10 w-10 rounded-xl bg-gradient-to-br ${c.color} text-white flex items-center justify-center text-sm font-extrabold shrink-0`}
              >
                {c.initials}
              </div>
    ...
    <section id="caracteristicas" className="mx-auto max-w-6xl px-6 py-24">
      <div className="max-w-2xl">
        ...
        <p className="mt-4 text-ink-variant">
          Diseñado junto a empresas que viven día a día la facturación, las
          devoluciones y el cuadre de caja. Cada métrica está pensada para que
          actúes, no solo para que mires.
        </p>
      </div>
      <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {items.map(({ icon, title, desc }) => (
          <div key={title} className="card-bordered p-6">
            ...
            <p className="mt-2 text-sm text-ink-variant leading-relaxed">
              {desc}
            </p>
          </div>
        ))}
      ...
      <div className="mx-auto max-w-6xl px-6 py-24">
        <div className="flex items-end justify-between flex-wrap gap-6">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 text-4xl font-extrabold tracking-tight">
              Los datos que ya conoces, en una experiencia moderna.
            </h2>
          </div>
          <Link href="/login" className="cta !py-2.5 !px-5 text-sm">
            Entrar al panel
        ...
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {reports.map(({ icon, title, desc }) => (
            <div key={title} className="card-bordered p-6">
              ...
              <p className="mt-4 text-sm text-ink-variant leading-relaxed">
                {desc}
              </p>
            </div>
          ))}
  ...
  return (
    <section id="como-funciona" className="mx-auto max-w-6xl px-6 py-24">
      <div className="text-center max-w-2xl mx-auto">
        ...
        <h2 className="mt-3 text-4xl font-extrabold tracking-tight">
          De tus datos crudos a decisiones, en tres pasos.
        </h2>
        <p className="mt-4 text-ink-variant">
          Diseñado para que cualquier persona del equipo pueda leerlo, no solo
          el área de finanzas.
        </p>
      </div>
      <div className="mt-12 grid md:grid-cols-3 gap-5">
        {steps.map(({ icon, title, desc }, i) => (
          <div key={title} className="card-bordered p-7 relative">
            <span className="absolute top-5 right-5 text-5xl font-extrabold text-surface-mid leading-none select-none">
              0{i + 1}
            </span>
  ...
  return (
    <section id="precios" className="mx-auto max-w-6xl px-6 py-24">
      <div className="text-center max-w-2xl mx-auto">
        ...
        <h2 className="mt-3 text-4xl font-extrabold tracking-tight">
          Planes simples para empresas que crecen.
        </h2>
        <p className="mt-4 text-ink-variant">
      ...
      <div className="mt-12 grid md:grid-cols-3 gap-5">
        {plans.map((p) => (
          <div
            key={p.name}
            className={`card-bordered p-8 flex flex-col ${
              p.featured
                ? "border-primary/30 shadow-cta ring-1 ring-primary/20"
                : ""
            }`}
          >
            {p.featured && (
              <span className="pill self-start bg-primary text-white mb-4">
                Más popular
              </span>
            )}
            ...
            <div className="mt-6 flex items-baseline gap-1">
              ...
              {p.price.startsWith("$") && (
                <span className="text-sm text-ink-variant">/ mes</span>
              )}
            </div>
            ...
            <Link
              ...
              className={`mt-8 inline-flex items-center justify-center rounded-pill px-5 py-3 text-sm font-bold transition ${
                p.featured
                  ...
                  : "bg-surface-low text-ink hover:bg-surface-mid"
              }`}
            >
              Iniciar sesión
            </Link>
      ...
      <div className="rounded-card bg-primary-gradient p-10 sm:p-14 text-white flex flex-col lg:flex-row lg:items-center gap-8 lg:justify-between">
        <div className="max-w-2xl">
          <h3 className="text-3xl sm:text-4xl font-extrabold tracking-tight">
            ¿Listo para ver tus reportes con claridad?
          </h3>
          <p className="mt-3 text-white/85">
            Accede a tu panel y descubre lo que tus datos pueden contarte hoy.
          </p>
        </div>
        <Link
          href="/login"
          className="inline-flex items-center justify-center gap-2 rounded-pill bg-white px-7 py-4 text-sm font-bold text-primary hover:bg-white/90 transition"
        >
          Iniciar sesión
          <Icon name="arrow_forward" size={16} />
        </Link>
    ...
    <footer className="border-t border-surface-mid bg-surface-lowest">
      <div className="mx-auto max-w-6xl px-6 py-10 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <p className="text-xs text-ink-variant">
          © {new Date().getFullYear()} Zempac Analitika. Todos los derechos
          reservados.
        </p>
      </div>
    </footer>
