...
> **`npm run lint` does not work** (found 2026-08-05). Two independent breakages:
> `next lint` was **removed in Next 16**, so the script errors with "Invalid project
> directory provided, no such directory: …/lint"; and the repo still has an
> `.eslintrc.json`, which ESLint 9 only reads via `ESLINT_USE_FLAT_CONFIG=false` —
> and under that compat path `eslint-config-next` crashes with "Converting circular
> structure to JSON". Fixing it means migrating to a flat `eslint.config.mjs`.
> **Until then `npm run build` is the only real gate** — it runs the TypeScript
> check. `npx tsc --noEmit` is the fast inner-loop equivalent.
...
> **Cuadre de Caja — pestaña "Productos por lotes"** (2026-07-02) — third tab in `src/app/dashboard/cuadre-caja/page.tsx`. Shares the lote master list with "Por lotes" via a `LoteMasterDetail` scaffold (the tab injects the detail through a `renderDetail(lote, ctx)` prop); the detail is a Código / Producto / Vendido table. Endpoint: `GET /api/reportes/analitica-productos-por-lote/{lote}?orderBy=N` (lote = `NIR`; rows are `{ Codigo, Nombre, Vendido }`; `orderBy` 1=código asc, 2=nombre asc, 3=vendido desc). **The SP ignores `orderBy`** (every value returns the same order), so we fetch **once per lote** (cache key free of orderBy) and sort **client-side** via the sort pills. `Codigo` is rendered raw (it's a code, not a quantity — no `fmtInt`). Types `RptProductoPorLote` / `parseProductoPorLote` + `PRODUCTO_LOTE_ORDER` in `src/lib/types.ts`; `apiAnaliticaProductosPorLote` in `src/lib/api.ts`. Mirrored in Flutter (`lib/screens/cuadre_caja/cuadre_caja_screen.dart`, `lib/models/rpt_lote.dart`).

> **Productos Negativos is now live** (2026-07-09) — `src/app/dashboard/productos-negativos/page.tsx`. Server-paginated report of products with negative stock (existencia). **`sucursal` is REQUIRED** (>0 — the SP returns `400 "El ID de sucursal debe ser mayor que cero"` without it); the page auto-selects the first sucursal (**no "Todas" option**) and resets to page 1 on switch. The response is a **paginated envelope** `{ pagina, porPagina, totalRegistros, totalPaginas, data }` (not a bare array like the other reports); each row is `{ DocNum, Nombre, Almacen, Existencia }` — the per-row `TotalRegistros` is an SP artifact and is ignored in favor of the envelope total. Page size 20 with prev/next controls; each page+sucursal is its own `useApi` key `rpt:productos-negativos:<sucursal>:<pagina>:<porPagina>`. `DocNum` is rendered raw (a code, not a quantity); `Existencia` via `fmtDecimal` in the negative `text-tertiary` token. Types `RptProductoNegativo` / `ProductosNegativosPage` + `parseProductoNegativo` / `parseProductosNegativosPage` in `src/lib/types.ts`; `apiProductosNegativos` in `src/lib/api.ts`. NAV entry at the **end of the flat list** (icon `warning`) + `global-search` index entry. Mirrored in Flutter (`lib/screens/productos_negativos/productos_negativos_screen.dart`, `lib/models/rpt_producto_negativo.dart`, route `/productos-negativos` under the "Reportes Avanzados" drawer section).

> **Sobre Stock is now live** (2026-08-04) — `src/app/dashboard/sobre-stock/page.tsx`. Top-N products with the most days of inventory among those that sold in the analysis window. Endpoint: `GET /api/reportes/sobre-stock-productos?diasAnalisis&topN&umbralDiasSobreStock&ordenarPor&sucursalId` — **every param is optional** (the SP has its own defaults) and `sucursalId` is omitted for "Todas las sucursales". **`ordenarPor` (1 = mayor sobre stock, 2 = más vendidos, UI default 2) is a filter, not a display sort**: combined with `topN` it decides *which* rows come back, so it lives in the filter card. Because of it the list's own sort pills default to `'api'` (`ORDENES[0]`, "Orden del reporte"), which returns the rows untouched — any other default would silently override the criterion the user just picked. Three pill groups drive it: ventana `30/90/180` (default 90), umbral `30/45/60/90` (**default 30** — the SP's own fallback is 45, but the UI is deliberately stricter and always sends the param explicitly), top `10/20/50/100` (default 20). Their labels deliberately avoid the endpoint's parameter names — "Ventas de los últimos", **"Sobre stock: más de"** (the direction has to be explicit: the umbral is the *floor* of the problem zone, and "desde"/"hasta" phrasings get read backwards), "Mostrar"; all four filters are in the cache key `rpt:sobre-stock:<sucursal>:<dias>:<umbral>:<top>`. `Estado` is `SOBRE STOCK` or `NORMAL`; unknown values render verbatim in a neutral chip and count as over-stock. All derived arithmetic (excedente, capital inmovilizado, severidad, orden, `coberturaLegible`) lives in [`src/lib/sobre-stock.ts`](src/lib/sobre-stock.ts), deliberately React-free. **Row bars show each product's share of the list's capital inmovilizado.** An earlier version anchored them to `5 ×` the umbral, but real data has products at `1,300 ×` the umbral, so every bar rendered 100% full and the umbral marker was meaningless — capital share is bounded and actually discriminates. For the same reason `DiasDeInventario` is rendered in **años past two years** (58,950 días reads as 161 años) and the `× el umbral` chip was dropped. Types `RptSobreStockProducto` / `parseSobreStock` in `src/lib/types.ts`; `apiSobreStockProductos` in `src/lib/api.ts`; Excel via `ExcelReportKey 'sobre-stock-productos'` (**note the export takes `top`, not `topN`**). NAV icon: `inventory_2`. **Not yet mirrored in Flutter** — see `docs/superpowers/specs/2026-08-04-sobre-stock-productos-design.md` §7.
